!======================================================================!
!     Title  : CWF_test.f90                                            !
!     Date   : 2009-3-1-Sun ~ 2009-3-7-Sat                             !
!     Modified by : Yusa Shusaku                                       !
!     Coded by : ???                                                   !
!                                                                      !
!     Module for Coulomb wave function.                                !
...
!     call dfcoul(eta, rho, fcw, gcw, gpcw, sigmad, L. iexp)           !
!                                                                      !
!     eta  --- Sommerfeld parameter.(real(8), intent(in))              !
!     rho  --- Independent variable.(real(8), intent(in))              !
!     fcw(1:L) --- Regular Coulomb wave function of angular momentum L.!
!                  (real(8), dimension(1:L), intent(out))              !
!     fpcw(1:L) --- Derivative of regular Coulomb wave function.       !
!                 (real(8), dimension(1:L),intent(out))                !
!     gcw(1:L)  --- Irregular Coulomb wave function. intent(out)       !
!     gpcw(1:L) --- Derivative of irregular Coulomb wave function.     !
!                 (real(8), dimension(1:L), intent(out)                !
!     sigmad(1:L) --- Coulomb phase shift.                             !
!                  (real(8), dimension(1:L), intent(out))              !
...
!! At first, kr = 1. We can get out the loop if the kr becomes 3.
      do
...
!       translation inverse
        if (i >= 1 .or. (i == 1 .and. xd /= 0.0d0)) then
          i = i - 1
          x = i
          x = xx + x
          cycle
...
!!      controle du quadrant
        if (xd == 0.0d0) then
          tra = pi * y
          if (tra <= 1.0d-2) then
            par=tra*(2.0d0+tra*(-2.0d0+tra*(1.333333333333333d0+tra*( &
     &        -0.6666666666666666d0+tra*(0.2666666666666666d0+tra*(   &
     &        -0.08888888888888888d0+tra*0.02539682539682540d0))))))
            tra1 = - log(y) - log(par)
          else
            par = 1.0d0 - exp(- tra - tra)
            tra1 = - log(y * par)
          end if
          par = 0.5d0 * (alo2pi - tra + tra1)
          pai = pai - pis2
...
!       x+iy change en -x-iy
        ...
        tra = exp(- tra - tra)
        x = pi * mod(x, 2.0d0)
        sini = (1.0d0 - tra) * cos(x)
        cosi = (1.0d0 + tra) * sin(x)
        kr = 3
        x = abs(cosi)
        y = abs(sini)
      end do
...
!       conditions d existence
        write (6,*) 'jflgam(0) est infini'
        par = 1.0d74
        nbchif = 0
        return
      else if (xd < 0.0d0) then
        if (x >= 4503599627370496.0d0) then
          write (6,*) 'argument de jflgam trop grand'
          par = 1.0d74
          nbchif = 0
          return
        end if
        y = mod(x, supint)
        if (y == 0.0d0) then
          write (6,*) 'jflgam (-entier) est infini'
          par = 1.0d74
          nbchif = 0
          return
        else
